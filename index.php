    ...
    if($action=='lunch_order'){
		    	...
		    	<?php
				    ...
				    <table  class="food_table">
				    	...
				    	<?php
						$db = connect_pdo();$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
						$sql = "DROP TABLE IF EXISTS `res_days`";
						$stmt = $db->query($sql); 

						$sql = "CREATE TABLE `res_days` (
							`id` int(11) NOT NULL,
							`res_id` int(11) NOT NULL,
							`res_name` text NOT NULL,
							`days_since` text NOT NULL
						  ) ENGINE=InnoDB DEFAULT CHARSET=latin1";
						$stmt = $db->query($sql);  

						$sql = "ALTER TABLE `res_days`
						ADD PRIMARY KEY (`id`)";
						$stmt = $db->query($sql); 
					    ...
					    if($rows){
							    foreach($rows as $row){
									$days_since = days_since($row['id']);
									$sql = "INSERT INTO res_days (res_id,res_name,days_since) VALUES (:res_id, :res_name, :days_since)";
									$stmt = $db->prepare($sql) ;
										$stmt->execute(array(
												'res_id' => $row['id'],
												'res_name' => $row['res'],
						...
						$sql = "SELECT * FROM res_days order by days_since + 0";
					    $stmt = $db->query($sql);
						$rows = $stmt->fetchAll();
						foreach($rows as $row){
								  ?>
								<tr>
									<?php if (check_if_today($row['res_id'])){$background = "#52DC6A";}else{$background = '';} ?>
									<td style="background:<?=$background;?>;"><?=$row['res_name'];?></td>
									<td style="text-align:center;"><?=$row['days_since'];?></td>
									<td><a class="edit button screen_only" title="Choose for today" href="lunch_finish.php?action=choose&res_id=<?=$row['res_id'];?>">Choose Today</a></td>
									<td><a class="edit button screen_only" title="Remove Chosen for today" href="lunch_finish.php?action=unchoose&res_id=<?=$row['res_id'];?>">UnChoose</a></td>
									<td><span class="edit button screen_only" title="Edit this entry">Edit</span></td>
									<td><a class="button screen_only" title="Menu Link" href="<?=get_menu($row['res_id']);?>" target="_blank">Menu</a></td>
							 	 ...
							 	 <tr class="individual_edit">
							 	 	<td colspan="5" style="padding-top:10px;">
							 	 			<form class="individual_item_form" method="post" action="lunch_finish.php?action=update_res" autocomplete="off">
								    	   		<input type="text" name="res" value="<?=$row['res_name'];?>">
									    		<input type="hidden" name="id" value="<?=$row['res_id'];?>">
												<input type="submit" value="Change Name">
											</form>
											<form method="post" action="lunch_finish.php?action=add_res_record&res_id=<?=$row['res_id'];?>"  style="display:inline;">
											...
											<form method="post" action="lunch_finish.php?action=add_menu&res_id=<?=$row['id'];?>" style="display:inline;">
													<input type="text" value="<?=get_menu($row['res_id']);?>" name="menu"  style="width:200px;" autocomplete="off">
			...
			function get_menu($res_id){
				$db = connect_pdo();$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$sql_menu = "SELECT * from restaurant where id = '$res_id'";
				$stmt = $db->query($sql_menu);
				$rows_menu = $stmt->fetchAll();
				$row_menu = $rows_menu[0];
				$menu = $row_menu['menu'];
				return $menu;
